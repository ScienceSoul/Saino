...
-(void)FEMKernel_solveLinearSystem:(FEMSolution *)solution {
    ...
    [self FEMKernel_computeChange:solution :NO :&n :varContainers->Values :NULL];
...
-(void)FEMKernel_setElementLoads:(FEMModel *)model: (FEMSolution *)solution: (Element_t *)element: (NSArray *)values: (NSMutableString *)name: (int*)indexes: (BOOL *)doneLoad: (int)n: (int)dof: (int)ndofs {
    ...
    if (dof >= 0) {
        ...
        if (valueList != nil) {
            valContainers = valueList.getContainers;
            workA = d3tensor(0, valContainers->sizeFValues1-1, 0, valContainers->sizeFValues2-1, 0, n-1);
        }
        ...
        for (j=0; j<n; j++) {
            ...
            if (k >= 0) {
                if (dof >= 0) {
                    ...
                    for (l=0; l<min(ndofs, valContainers->sizeFValues1); l++) {
                        k1 = ndofs * k + l;
    ...
    if (workA != NULL) {
        free_d3tensor(workA, 0,  valContainers->sizeFValues1-1, 0,  valContainers->sizeFValues2-1, 0, n-1);
        workA = NULL;
        valContainers = NULL;
    }
            ...
            for (j=0; j<n; j++) {
                ...
                if (k >= 0) {
                    if (dof >= 0) {
                        ...
                        for (l=0; l<min(solution.variable.dofs, valContainers->sizeFValues1); l++) {
...
*****************************************************************************************************************/
    ...
    if ([listUtil listGetLogical:model inArray:[boundaryConditionAtId returnValuesList] forVariable:str1 info:stat] == YES) {
        ...
        if (a != b) {
            ...
            n = solution.variable.dofs*projector.numberOfRows;
            a.numberOfRows = n;
        ...
        for (bc=0; bc<[model numberOfBoundaryConditions]; bc++) {
            ...
            @autoreleasepool {
                ...
                for (t=[model numberOfBulkElements]; t<[model numberOfBulkElements]+[model numberOfBoundaryElements]; t++) {
                    ...
                    [self FEMKernel_setElementLoads:model :solution :&elements[t] :[boundaryConditionAtId returnValuesList] :loadName :indexes :doneLoad :n :dof :solution.variable.dofs];
                }
            }
        }
    }
    ...
    @autoreleasepool {
        ...
        for (bc=0; bc<[model numberOfBoundaryConditions]; bc++) {
            ...
            if (nodesFound == NO) {
                ...
                if (valueList != nil) {
                    valContainers = valueList.getContainers;
                    coordNodes = doublematrix(0, valContainers->sizeFValues1-1, 0, valContainers->sizeFValues2);
                }
            ...
            if (coordNodes != NULL) {
                free_dmatrix(coordNodes, 0, valContainers->sizeFValues1, 0, valContainers->sizeFValues2);
                coordNodes = NULL;
                valContainers = NULL;
            }
            ...
            if (nodesFound == YES) {
                ...
                [self FEMKernel_setPointLoads:model :solution :elements :[boundaryConditionAtId returnValuesList] :loadName :nodeIndexes :n :dof :solution.variable.dofs];
