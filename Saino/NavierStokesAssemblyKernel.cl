...
#ifdef KERNEL_FP_32
    if (detG <= CL_FLT_MIN) {
        return; // Something is going wrong (probably degenerated element)
    };
#elif KERNEL_FP_64
    if (detG <= CL_DBL_MIN) {
        return;
...
__kernel void NavierStokesCompose(__global REAL *BasisFunctions,
                                  ...
                                  __constant int numberOfNodes,
                                  __constant int numberElementDofs,
                                  __constant int nBasis,
                                  __constant int varDofs) {
    ...
    REAL c2, c3, detJ, mu, muder0, muder, rho, u, v, w, s, ss, sum;
    ...
    for (int t=0; t<numberOfNodes; t++) { // Nb of Gauss points is equal to the number of nodes
        ...
        basis3D (BasisFunctions, basis, u, v, w);
        detJ = dBasisdx3D(BasisFunctions, elementNodeIndexesStore, nodesX, nodesY, nodesZ, dBasisdx,
                   u, v, w, numberOfNodes, numberElementDofs, globalID);
